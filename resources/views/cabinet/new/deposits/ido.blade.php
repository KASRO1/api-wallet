...
@section('content')
    ...
    <section id="invest-ftx">
        <div class="container">
            <div class="row">
                <div class="col-12">
                    <h1 class="section__heading py-4 mb-4"><span class="c-violet">IDO</span></h1>
                </div>
            </div>
            <form action="{{route('cabinet.deposits.store')}}" method="POST">
                ...
                <div class="row">
                    ...
                    <div class="col-md-6">
                        @include('cabinet.new.deposits._idoCurrencySelect')
                    </div>
                </div>
                <div class="row">
                    <div class="col-12 py-4 my-4">
                        <label for="amount-input">{{__("Enter amount")}} ({{__("Available amount")}}: ${{ $amount }})</label> <br>
                        @if($errors->any())
                        ...
                        <input type="text" @if($amount <= 0) disabled @endif value="0" class="wallet-item__info-box w-100" name="amount" >
                ...
                <div class="row mb-4">
                    ...
                    <div class="col-md-6">
                        <div class="time-helper choose-helper my-md-0 my-4">
                            <input type="number" name="days" hidden id="time_frame_input" value="7">
                            <div class="choose-helper__item time-helper__item">
                                <button data-time="7" class="choose-helper__btn time-helper__btn active">7 {{__("days")}}</button>
                            </div>
                            <div class="choose-helper__item time-helper__item">
                                <button data-time="14" class="choose-helper__btn time-helper__btn">14 {{__("days")}}</button>
                            </div>
                            <div class="choose-helper__item time-helper__item">
                                <button data-time="21" class="choose-helper__btn time-helper__btn">21 {{__("days")}}</button>
                            </div>
                            <div class="choose-helper__item time-helper__item">
                                <button data-time="28" class="choose-helper__btn time-helper__btn">28 {{__("days")}}</button>
                            </div>
                        </div>
                    </div>
                ...
                <div class="row">
                    <div class="col-12">
                        <button type="submit" @if($amount <= 0) disabled @endif id="submit-invest" class="btn btn--primary-gr">{{__("Invest now")}}</button>
                    </div>
                </div>
            </form>
        </div>
